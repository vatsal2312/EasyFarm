...
contract EasyFarmToken is ERC20, Ownable {
    using SafeMath for uint256;
    uint256 public supplyRemain = 1e27;
    ...
    modifier onlyMinter() {
        require(isMinter[msg.sender] == true, "permission denied");
        _;
    }

    function mint(address _to, uint256 _amount) public onlyMinter {
        if(supplyRemain > 0){
            if(supplyRemain >= _amount){
                _mint(_to, _amount);
                supplyRemain = supplyRemain.sub(_amount);
                _moveDelegates(address(0), _delegates[_to], _amount);

            }else{
                _mint(_to, supplyRemain);
                supplyRemain = 0;
                _moveDelegates(address(0), _delegates[_to], supplyRemain);
            }
        }
    }
